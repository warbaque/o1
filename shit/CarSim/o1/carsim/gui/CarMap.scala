...
class CarMap(tileSource: TileSource, center: Location, zoomLevel: Int) extends MapPanel(tileSource) {
  thisMap =>
  ...
  private def carAt(pixel: Point) = {
    ...
    def distance(carThread: CarThread) = {
      ...
      val carPixel = this.coordinatesToPixel(location.lat, location.long, false)
  ...
  private def attemptDrive(car: CarThread, pixel: Point): Unit = {
    val location: Location = this.pixelToCoordinates(pixel)
    this.attemptDrive(car, location.toString)
  }
...
private object CarMap {
  ...
  implicit def locationToMapViewerCoordinate        (location: Location): Coordinate = new Coordinate(location.lat, location.long) 
  implicit def mapViewerCoordinateToLocation        (coords: Coordinate): Location   = Location(coords.getLat, coords.getLon)
