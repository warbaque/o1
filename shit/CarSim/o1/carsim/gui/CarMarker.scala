...
class CarMarker(val map: CarMap, val car: CarEnhancement) extends Marker(null, null, car.location, 5, MapMarker.STYLE.FIXED, CarMarker.fuelStyle(car)) {
  ...
  override def toString = {
    "Car at (%.3f,%.3f). Fuel at %.1f%%. Driven: %.1f km".formatLocal(Locale.US, this.coordinates.lat, this.coordinates.long, this.car.fuelRatio.abs, this.car.kilometersDriven) 
  }
