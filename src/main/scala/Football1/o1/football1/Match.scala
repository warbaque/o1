...
class Match(val home: Club, val away: Club) {
  ...
  def isGoalless = (this.homeCount + this.awayCount) == 0
  ...
  def toString = {
    ...
    if (this.isGoalless)
      ...
      clubNamesAndStadium + "tied at " + this.homeCount + "-all"
    else if (this.goalDifference > 0 )
      clubNamesAndStadium + this.homeCount + "-" + this.awayCount + " to " + this.home.name
    else
      clubNamesAndStadium + this.awayCount + "-" + this.homeCount + " to " + this.away.name
  }
}
