...
object Scenario {
  ...
  abstract class MapScenario(name: String) extends Scenario(name) {
    ...
    private def addWalls(world: RobotWorld) = {
      val wallLocs = for {
        ...
        if (this.map(y)(x) == '#')
      } yield new Coords(x + 1, y + 1)
      wallLocs.foreach( world.addWall(_) )
    }

    protected def addBot(world: RobotWorld, x: Int, y: Int, facing: Direction) = {
      world.addRobot(new Coords(x + 1, y + 1), facing)
    }
  ...
  object Bar extends MapScenario("Bar") {
    ...
    protected def addBots(world: RobotWorld) = {
      ...
      val flyLocs = for {
        ...
        coords = new Coords(col + 1, row + 1)
        if (world(coords).isEmpty && (row + col) % 3 != 2)
      } yield coords
      var flyCount = 0
      for (loc <- flyLocs) {
        flyCount += 1
        val fly = world.addRobot(loc, Direction.Clockwise(flyCount % 4))
  ...
  object Termination extends MapScenario("Termination") {
    ...
    protected def addBots(world: RobotWorld) = {
      o1.soita("[40]<<<" + "(d)-----------------------" * 2 + "&P:<<" + "(dC)(dC) (dC) (dC)(dC)     " * 2 + "(DCCCCe<E>)(DCCCCe<E>) (DCCCCe<E>) (DCCCCe<E>)(DCCCCe<E>)     " * 2 + "/180")
  ...
  abstract class RandomScenario(name: String) extends Scenario(name) {
    ...
    def makeDirection() = {
      Direction.Clockwise(util.Random.nextInt(Direction.Clockwise.size))
    }
  ...
  abstract class BasicRandomScenario(name: String, val acceptedTypes: Seq[RobotType]) extends RandomScenario(name) {
    ...
    protected def makeWorld(width: Int, height: Int, walls: Int, robots: Int) = {
  ...
  class RandomCustom(name: String, acceptedTypes: Seq[RobotType]) extends BasicRandomScenario(name, acceptedTypes) {
    ...
    def setup(locator: Component) = {
      ...
      } yield this.makeWorld(width, height, walls, robots)
    }
  ...
  class RandomDefault(name: String, size: Int) extends BasicRandomScenario(name, RobotType.All.filterNot( _ == Psychobot )) {
    def setup(locator: Component) = {
      Some(this.makeWorld(size, size, (size * size) / 25, (size * size) / 6))
    }
  }
