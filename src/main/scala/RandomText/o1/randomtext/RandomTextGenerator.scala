...
class RandomTextGenerator(coherence: Int) {
  ...
  /** Returns a fragment of random text generated based on the text in the indicated plain text file. 
    ...
    *                         (project folder) or a web address pointing to a plain text file somewhere on the web.
  ...
  private def createStreamOfNonsense(data: String) = {
    ...
    val starts = if (this.depth > 1) {
      val possibleStarts = data.sliding(this.depth - 1).toSeq
    ...
    def nonsense(previous: String): Stream[Char] = {
    ...
    nonsense(starts(Random.nextInt(starts.length)))
  }
...
private class CharDistribution(data: String, depth: Int) {
  ...
  private val cumulativeFreqsByPrelude = {
    ...
    val allSlicesInData        = data.sliding(depth).toSeq
    val sliceFreqs             = allSlicesInData.groupBy(identity).mapValues( _.size )
    val sliceFreqsByPrelude    = sliceFreqs.groupBy(tupled(prelude))
    val followerFreqsByPrelude = sliceFreqsByPrelude.mapValues(followerFreqs)
    followerFreqsByPrelude.mapValues(cumulativeFrequency)
  }
  ...
  def generateRandomChar(precedingChars: String) = { 
    ...
    cumulativeFreqs.from(Random.nextInt(max) + 1).head match {
      case (freq, randomChar) => randomChar
    }
  }
} 
